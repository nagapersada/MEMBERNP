...
function calculateMyRank(currentTeamSize, directDownlineCount, uid) {
    const rankTiers = [
        { name: "V.I.P 9", level: 9, min: 3501, max: Infinity, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 8", level: 8, min: 1601, max: 3500, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 7", level: 7, min: 901, max: 1600, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 6", level: 6, min: 501, max: 900, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 5", level: 5, min: 351, max: 500, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 4", level: 4, min: 201, max: 350, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 3", level: 3, min: 101, max: 200, reqVip: 2, reqLevelName: "V.I.P 2" }, 
        { name: "V.I.P 2", level: 2, min: 31, max: 100, reqVip: 2, reqLevelName: "V.I.P 1" },
    ...
    // Default status jika sudah maksimal
    ...
    // Perhitungan Target Next
    else if (nextRank) {
        
        // 1. Prioritas utama: Syarat Total Anggota
        ...
        // Khusus V.I.P 1, gunakan Direct Downline Count
        if (nextRank.level === 1) {
             nextGapValue = nextRank.min - directDownlineCount;
             descHtml = `Direct Downline lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju ${nextRank.name}</span>`;
        }

        // 2. Tampilkan Status Struktur di Rank Card (Tidak mempengaruhi Target Next Value/Gap)
        let isStructureMet = true;
        let requiredVips = 0;
        let currentVips = 0;
        let reqLevelName = "";

        if (nextRank.level >= 3) {
            requiredVips = nextRank.reqVip;
            currentVips = vip2InTeam;
            reqLevelName = nextRank.reqLevelName;
        ...
        if (currentRankLevel >= 1 && currentVips < requiredVips) {
            isStructureMet = false;
            // UPDATE RANK STATUS UNTUK MENUNJUKKAN SYARAT STRUKTUR YANG BELUM TERPENUHI
            let structureGap = requiredVips - currentVips;
            document.getElementById('rankNextGoal').innerText = `Tambahan ${structureGap} @${reqLevelName} dalam Tim`;
            document.getElementById('rankNextGoal').style.color = '#ff4444'; // Opsional: Beri warna merah untuk warning

            // Namun, Target Next Value/Gap tetap menunjukkan pertumbuhan anggota
            // (sudah dihitung di langkah 1)
        } else {
            document.getElementById('rankNextGoal').style.color = '#ccc'; // Kembalikan warna default jika syarat struktur terpenuhi
        }
    }
    
    // Khusus MEMBER (currentRankLevel 0)
    ...
    // Periksa jika Target Next (Anggota Tim) sudah terpenuhi, 
    // tapi Status di Rank Card masih menunjukkan kekurangan struktur.
    if (currentRankLevel >= 1 && document.getElementById('rankNextGoal').innerText.includes("Tambahan") && nextGapValue <= 0) {
        // Jika syarat anggota sudah OK, tapi syarat struktur belum,
        // tampilkan 0 di Target Next Gap, karena pertumbuhannya sudah cukup.
        document.getElementById('nextLevelGap').innerText = 0;
        nextGoalText = `Menuju ${nextRank.name}`;
        descHtml = `Anggota Tim lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju ${nextRank.name}</span>`;
    }

    // Set nilai akhir Target Next
    document.getElementById('rankNextGoal').innerText = nextGoalText; // Ini yang diubah di Rank Card
    document.getElementById('nextLevelGap').innerText = Math.max(0, nextGapValue); 
