...
let newVipAchievers = {1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[], 8:[], 9:[]}; // Data Notifikasi
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Load Data jika sudah login (untuk dashboard/list/network)
    if (isLoggedIn) await loadData();

    // Event Listeners sesuai Halaman
...
// === FUNGSI LOAD DATA (DIPERBAIKI AGAR TIDAK STUCK) ===
async function loadData() {
    try {
        // Ambil data dari Supabase
        const { data, error } = await db.from('members').select('*');
        
        // Jika error dari Supabase, lempar ke Catch agar terdeteksi
        if (error) throw new Error(error.message);

        // Jika koneksi sukses tapi data kosong
        if (!data || data.length === 0) {
            console.warn("Koneksi sukses, tapi tabel kosong.");
            globalData = [];
            return;
        }

        // Mapping Data (Normalisasi Huruf Besar/Kecil Kolom)
        globalData = data.map(item => ({
            uid: String(item.UID || item.uid || '').trim(),
            name: (item.Nama || item.nama || item.name || '-').trim(),
            upline: item.Upline || item.upline ? String(item.Upline || item.upline).trim() : "",
            joinDate: new Date(item.TanggalBergabung || item.tanggalbergabung || item.joinDate)
        }));

    } catch (err) {
        console.error("GAGAL LOAD DATA:", err);
        // Kita biarkan error ini agar fungsi pemanggil (doLogin) tahu ada masalah
        throw err; 
    }
}

// === FUNGSI LOGIN (DIPERBAIKI AGAR MUNCUL ALERT JIKA ERROR) ===
async function doLogin() {
    ...
    if (!inputUid) {
        errorMsg.innerText = "Masukkan UID Anda";
        return;
    }
    ...
    try {
        ...
        // Cari UID di data yang sudah di-load
        ...
        if (user) {
            ...
            // UID TIDAK DITEMUKAN
            errorMsg.innerText = "UID Tidak Terdaftar di Database";
            // Reset Tombol
            btn.innerText = "MASUK";
            btn.disabled = false;
            btn.style.opacity = "1";
        ...
        // JIKA KONEKSI GAGAL (Supaya tidak STUCK)
        alert("KONEKSI GAGAL: " + err.message + "\n\nCek sinyal internet atau Database Anda.");
...
// === LOGIKA DASHBOARD & FITUR LAIN (TETAP UTUH) ===
...
function renderDashboard(){
    checkNotificationPermission(); // Cek Izin Notifikasi
    ...
    const g=[b,...d];
    myTeamData = g; 
    countVipStats(g); // Hitung VIP & Notifikasi
...
function getRankLevel(a){
    ...
    // Handle jika data member tidak ketemu (misal load belum selesai)
    ...
    for(const tier of rankTiers) {
        if (totalMembers >= tier.min) {
            let currentVips = 0;
            if (tier.level >= 3) currentVips = vip2InTeam;
            else if (tier.level === 2) currentVips = vip1InTeam;
...
function countVipStats(a){
    ...
    let storedVips = JSON.parse(localStorage.getItem('savedVipData')) || {1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[], 8:[], 9:[]};
    let hasNotification = false;

    for(let i=1; i<=9; i++){
        const countElem = document.getElementById(`cVIP${i}`);
        const boxElem = countElem ? countElem.parentElement : null;
        if(countElem) countElem.innerText = b[i];

        let oldUids = storedVips[i] || [];
        let currentUids = vipLists[i].map(m => m.uid);
        let newMemberUids = currentUids.filter(u => !oldUids.includes(u));

        if (newMemberUids.length > 0) {
            newVipAchievers[i] = newMemberUids;
            if(boxElem) boxElem.classList.add('vip-alert');

            if (!hasNotification) {
                showPushNotification(`Selamat! Ada anggota baru mencapai V.I.P ${i}`);
                hasNotification = true;
            }
        } else {
             if(boxElem) boxElem.classList.remove('vip-alert');
        }
    }

    if(Object.keys(storedVips).length === 0 && Object.keys(vipLists).length > 0) {
        updateVipLocalStorage();
        document.querySelectorAll('.vip-alert').forEach(el => el.classList.remove('vip-alert'));
    }
}
...
window.openVipModal = function(level) {
    ...
    const gridItems = document.querySelectorAll('.vip-item');
    gridItems.forEach(item => {
        if(item.getAttribute('onclick') === `openVipModal(${level})`) item.classList.remove('vip-alert');
    });

    title.innerText = `DAFTAR V.I.P ${level}`;
    body.innerHTML = ''; 
    const list = vipLists[level];
    const newUids = newVipAchievers[level] || [];

    if (list && list.length > 0) {
        list.forEach(m => {
            let nameClass = "v-n";
            let nameText = m.name;
            if (newUids.includes(m.uid)) {
                nameClass += " new-achiever-text";
                nameText += " (BARU!)";
            }
            body.innerHTML += `<div class="v-item"><span class="${nameClass}">${nameText}</span><span class="v-u">${m.uid}</span></div>`;
        });
    } else {
...
function calculateMyRank(currentTeamSize, directDownlineCount, uid) {
    ...
    let nextGapValue = 0, nextGoalText = "", descHtml = "";
    ...
    if (currentRankLevel === 9) structureGapText = "Top Level";
        ...
        let currentVips = (nextRank.level >= 3) ? vip2InTeam : (nextRank.level === 2 ? vip1InTeam : 0);
...
// --- DOWNLOAD TXT (FITUR PERMINTAAN LAMA) ---
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        if (d <= 15) { let prevM = m - 1, prevY = y; if (prevM < 0) { prevM = 11; prevY--; } startP = new Date(prevY, prevM, 16); endP = new Date(prevY, prevM + 1, 0, 23, 59, 59); cutoffBase = new Date(prevY, prevM, 16); periodLabel = `PERIODE 2 (${getMonthName(prevM)} ${prevY})`; } else { startP = new Date(y, m, 1); endP = new Date(y, m, 15, 23, 59, 59); cutoffBase = new Date(y, m, 1); periodLabel = `PERIODE 1 (${getMonthName(m)} ${y})`; }
        ...
        myTeamData.forEach(member => { if (member.joinDate >= cutoffBase) return; const downlines = getDownlinesRecursive(member.uid); const baseCount = downlines.filter(dl => dl.joinDate < cutoffBase).length; co
