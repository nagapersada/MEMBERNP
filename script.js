...
// Variabel Global untuk menyimpan Text Download
let achieverTxtContent = "";

// Variabel untuk menyimpan UID VIP Baru yang belum dilihat
...
function countVipStats(a){
    ...
    // Ambil Data Tersimpan (Local Storage)
    ...
    // Loop Level 1-9 untuk cek perbedaan data baru vs lama
    for(let i=1; i<=9; i++){
...
// FUNGSI MODAL POPUP (DIMODIFIKASI UNTUK FITUR KEDIP & SIMPAN DATA)
window.openVipModal = function(level) {
    ...
    const newAlerts = newVipAlerts[level] || []; // Daftar UID yang harus kedip

    if (list && list.length > 0) {
        list.forEach(m => {
            // Cek apakah member ini "Baru"? Jika ya, beri class blink-row
    ...
    // --- LOGIKA RESET & SIMPAN DATA ---
    // Setelah dibuka, kita anggap user sudah melihat. Hapus kedip di dashboard.
    if(btnElement) btnElement.classList.remove('blink-alert');

    // Update Local Storage dengan data saat ini (agar besok tidak kedip lagi untuk orang yang sama)
    let savedVipState = JSON.parse(localStorage.getItem('savedVipState')) || {};
    
    // Simpan daftar UID saat ini untuk level ini
    savedVipState[level] = vipLists[level].map(m => m.uid);
    
    localStorage.setItem('savedVipState', JSON.stringify(savedVipState));
    
    // Kosongkan alert sementara (karena sudah dilihat)
    newVipAlerts[level] = [];
}
...
// --- TAMBAHAN FITUR: PERAIH 50% PERIODE LALU & DOWNLOAD TXT ---
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        // Header Text untuk File Download
