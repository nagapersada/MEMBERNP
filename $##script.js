...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        // Jika Hari ini tgl 1-15 (P1) -> Maka cek Periode 2 Bulan Sebelumnya
        // Jika Hari ini tgl 16-31 (P2) -> Maka cek Periode 1 Bulan Ini
        if (d <= 15) {
            let prevM = m - 1;
            let prevY = y;
        ...
        // Gunakan myTeamData yg sudah disiapkan di renderDashboard
        ...
        if (achievers.length === 0) {
            body.innerHTML = '<div class="v-empty">Belum ada anggota yang mencapai target pada periode tersebut.</div>';
        } else {
            let html = '';
            achievers.forEach(a => {
                html += `
